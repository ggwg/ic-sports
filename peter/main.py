...
async def remote_render_worker(ball: Ball, hand: Hand, decoder: video.VideoDec, score: Score):
    while True:
        try:
            ...
            cv2.putText(frame, f"{score.my}:{score.competitor}", (video.IMAGE_WIDTH, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
...
async def main():
    ...
    with video.VideoDec() as decoder, video.VideoCap() as capture, encoder if client_or_server == "client" else dummy_resource():
        ...
        async with remote:
            ...
            asyncio.create_task(remote_render_worker(ball, hand, decoder, score))
